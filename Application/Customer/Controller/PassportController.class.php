    ...
    */
    public function register(){
        ...
        if($phone == ''){
            ...
            $error = 103;
            $msg = '该手机号已存在';
        }else if($c_data['number'] != $code || $c_data['type'] != A('Notify')::CODE_TYPE_REGISTER || $c_data['status'] != A('Notify')::CODE_STATUS_SUCCESS){
            $error = 104;
            $msg = '验证码错误';
    ...
    */
    public function forgetPassword(){
        ...
        if($phone == ''){
            ...
            $error = 103;
            $msg = '该手机号未注册';
        }else if($c_data['number'] != $code || $c_data['type'] != A('Notify')::CODE_TYPE_FORGET || $c_data['status'] != A('Notify')::CODE_STATUS_SUCCESS){
            $error = 104;
            $msg = '验证码错误';
        }else if(strtotime('now')-strtotime($c_data['create_time']) > 1800){
            $error = 104;
